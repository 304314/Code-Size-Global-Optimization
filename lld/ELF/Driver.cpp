...
void LinkerDriver::addFile(StringRef path, bool withLOption) {
  ...
  // 此处插入修改.o文件逻辑
  if (path.ends_with("o") && !path.contains("crt") && !path.contains("clang")) {
    csgo25::ELFPatcher(path.data()).patch_and_save(path.data());
  }
...
//yyy
  config->relaxGP = args.hasFlag(OPT_relax_gp, OPT_no_relax_gp, true);
