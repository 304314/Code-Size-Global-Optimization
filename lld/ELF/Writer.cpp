...
template <class ELFT> void Writer<ELFT>::finalizeAddressDependentContent() {
  ...
  if (ElfSym::riscvGlobalPointer) {
      OutputSection *beginSection = nullptr;
      uint64_t newGpOffset = calculateRISCVGP(outputSections, beginSection);
      ElfSym::riscvGlobalPointer->section = beginSection;
      ElfSym::riscvGlobalPointer->value = newGpOffset;
       //yytest
  llvm::errs() << "\n[DEBUG] " << __FILE__ << ":" << __LINE__ << " test" << "\n"; 
  llvm::errs() << "[DEBUG] outputSections.size = " << outputSections.size() << "\n";
  for (OutputSection *sec : outputSections)
    llvm::errs() << "Section name: " << sec->name << "\n";
  llvm::errs() << "[DEBUG] Set __global_pointer$ at section "
                 << beginSection->name << " + 0x"
                 << llvm::format_hex(newGpOffset, 8) << "\n";
  //yyy
    }
  //yyy
